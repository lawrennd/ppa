...
EOF
echo "ppaGunnarConvergence($2,$3,$4);" >> $1
echo exit >> $1
}	 
...
DIRNAME= #mlprojects/ppa/convergenceCode/
NAME=ppaGunnarConvergeRBF
RUNNAME=$NAME
KERN="{'rbf', 'white', 'bias'}"
DATASETS=banana #::thyroid:ringnorm:flare-solar:twonorm:german:waveform::breast-cancer:diabetis:german:heart:titanic
DATANUM=`seq -s: 2 5`
...
for dataNum in $DATANUM
  ...
  for dataset in $DATASETS
    ...
    for invWidth in $INVWIDTHPARAMS
      ...
      echo $MFILE
      echo $SFILE
      createMatlabFile "$MFILE" "'${dataset}'" "$dataNum" "$KERN"
