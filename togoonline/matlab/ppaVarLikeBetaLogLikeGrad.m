function g = ppaVarLikeBetaLogLikeGrad(model)

% PPAVARLIKEBETALOGLIKEGRAD Calculate the variational log-likelihood Gradient
% FORMAT
% DESC Calculate the variational log-likelihood Gradient.
...
for j = 1:size(fBarfBar,3)
    g = g + B(:,j).^-1 -(ff(:,j)-2.*f(:,j).*fBar(:,j)+diag(fBarfBar(:,:,j)));
end

g = 0.5.*g;
fhandle = str2func([model.betaTransform 'Transform']);
g = g.*fhandle(model.B, 'gradfact');
g=g';